  ...
  Future<void> _signInWithGoogle() async {
    try {
      ...
      if (user != null) {
        ...
        // Set role
        String userRole =
            user.uid == "MiDkxr5RAoRSHKJlIdV10WKIJDy2" ? "admin" : "user";
        await FirebaseDatabase.instance.ref("users/${user.uid}").update({
          "role": userRole,
        });

        // ✅ Update FCM Token di dalam 'schedule'
        final deviceRef = FirebaseDatabase.instance.ref(
          "users/${user.uid}/device",
        );
        final deviceSnapshot = await deviceRef.get();

        if (deviceSnapshot.exists) {
          for (final device in deviceSnapshot.children) {
            await deviceRef
                .child("${device.key}/schedule/fcmToken")
                .set(fcmToken ?? '');
          }
        }
